...
export default function App() {
  ...
  const fetchStock = useCallback(async () => {
    ...
    try {
      const [res, histRes] = await Promise.all([
        axios.get(`${RAILWAY_URL}/stock/${symbol}`),
        axios.get(`${RAILWAY_URL}/stock/${symbol}/history`)
      ]);
  ...
  const chartOptions = {
    ...
    scales: {
      x: { ticks: { color: '#555', maxTicksLimit: 5, font: { size: 11 } }, grid: { display: false }, border: { display: false } },
      y: { ticks: { color: '#555', font: { size: 11 }, callback: (v) => `$${v}` }, grid: { color: 'rgba(255,255,255,0.04)' }, border: { display: false } }
    }
  };
        ...
        <div className="search-wrapper">
          <div className="search-box">
            <input
              ...
              maxLength={10}
            />
            <button className="search-btn" onClick={fetchStock} disabled={loading}>
